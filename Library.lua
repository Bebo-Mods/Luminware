...
local L = {
    ...
    BgDeep   = Color3.fromRGB(13, 15, 23),
    BgMid    = Color3.fromRGB(19, 22, 33),
    BgLight  = Color3.fromRGB(27, 31, 46),
    SideCol  = Color3.fromRGB(15, 17, 26),
    TextHi   = Color3.fromRGB(232, 236, 255),
    TextMid  = Color3.fromRGB(140, 148, 175),
    TextLow  = Color3.fromRGB(68,  74,  100),
    ...
    Themes = {
        FROST={Accent=Color3.fromRGB(88,158,255), BgDeep=Color3.fromRGB(13,15,23),  BgMid=Color3.fromRGB(19,22,33),  BgLight=Color3.fromRGB(27,31,46),  SideCol=Color3.fromRGB(15,17,26)},
...
local function NewCard(L, col, title)
    ...
    function Card:AddToggle(idx,info)
        info=info or {}
        local row=Row(40); local lbl=LblL(row,info.Label or info.Text or idx)
        ...
        hit.MouseButton1Click:Connect(function() T:SetValue(not T.Value) end)
        ...
        T:Render(false); L.Toggles[idx]=T; return T
    end
    ...
    function Card:AddSlider(idx,info)
        ...
        L:N("TextLabel",{BackgroundTransparency=1,Font=L.Font,
            Text=info.Label or info.Text or idx,TextColor3=L.TextHi,TextSize=13,
            TextXAlignment=Enum.TextXAlignment.Left,
    ...
    function Card:AddButton(info)
        ...
        local row=Row(40); LblL(row,info.Label or info.Text or "")
    ...
    function Card:AddDropdown(idx,info)
        ...
        local DD={Value=info.Default,Values=info.Values or {},Type="Dropdown",Callback=info.Callback or function()end}
        ...
        local function build()
            ...
            for _,v in next,DD.Values do
                ...
                it.MouseButton1Click:Connect(function()
                    DD.Value=vv; sel.Text=tostring(vv); close(); build()
                    L:CB(DD.Callback,vv); L:CB(DD.Changed,vv)
                end)
                it.MouseEnter:Connect(function() if vv~=DD.Value then TW(it,{BackgroundTransparency=0.7},0.07) end end)
                it.MouseLeave:Connect(function() if vv~=DD.Value then TW(it,{BackgroundTransparency=0.9},0.07) end end)
            end
        ...
        function DD:SetValue(v) self.Value=v;sel.Text=tostring(v);build();L:CB(self.Callback,v);L:CB(self.Changed,v) end
    ...
    function Card:AddKeybind(idx,info)
        info=info or {}
        local row=Row(40); LblL(row,info.Label or info.Text or idx)
        ...
        kbtn.MouseButton1Click:Connect(function()
            ...
            local c; c=UIS.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.Keyboard then
                    KP.Value=i.KeyCode.Name; kbtn.Text=i.KeyCode.Name; picking=false; c:Disconnect()
        ...
        UIS.InputBegan:Connect(function(i)
            ...
            if i.UserInputType==Enum.UserInputType.Keyboard and i.KeyCode.Name==KP.Value then
...
function L:CreateWindow(cfg)
    cfg=cfg or {}
    local WW  = cfg.Size and cfg.Size.X or 900
    local WH  = cfg.Size and cfg.Size.Y or 560
    local logoId = cfg.Logo or ""
    local useAcr = cfg.Acrylic ~= false

    local Win={Tabs={},_tabOrder={}}
    ...
    self:N("TextButton",{BackgroundTransparency=1,Text="",Size=UDim2.fromScale(1,1),
        ZIndex=82,Parent=MinIcon}).MouseButton1Click:Connect(function()
        MinIcon.Visible=false; Root.Visible=true end)

    -- MAIN PANEL — ClipsDescendants keeps acrylic inside rounded corners
    ...
    SBtn("×",Color3.fromRGB(48,14,14),function()
    ...
    -- ── ADD TAB ─────────────────────────────────────────────────
    local NAV_ICONS={
        Aimbot="⊕", ESP="◎", Movement="⇥", Misc="☰", Settings="⚙",
    }
